...
class Elevator {
  ...
  toggleService() {
    if (this.state.status === 'moving') {
      return; // Don't toggle while moving
    }
    ...
    // Update the elevator appearance
  ...
  createElevatorElement() {
    ...
    // Add click handler for service toggle
  ...
  setStatus(status) {
    // Don't allow moving if out of service
    if (this.outOfService && status === 'moving') {
      return;
    }
...
class ElevatorManager {
  ...
  findClosestElevator(targetFloor) {
    ...
    this.elevators.forEach((elevator, index) => {
      if (elevator.outOfService) {
        return; // Skip out-of-service elevators
      }
  ...
  checkPendingCalls() {
    ...
    const availableElevatorIndex = this.elevators.findIndex(e => !e.outOfService && e.getStatus() !== 'moving');
